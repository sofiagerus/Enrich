/**
 * Bundle Edit Module - Bootstrap Dropdown Version
 */
...
const BundleEdit = (() => {
    ...
    function init(options) {
        ...
        // Debug submission
        const form = document.querySelector('.bundle-edit-form');
        if (form) {
    ...
    function renderCategoriesDropdown(categories) {
        ...
        const searchInput = elements.categoriesDropdown.querySelector('.dropdown-search-input');
        ...
        const render = (cats) => {
            ...
            cats.forEach(cat => {
                ...
                const item = createDropdownItem(cat.id, cat.name.split('(')[0].trim(), isSelected, (id, selected) => {
                    if (selected) { if (!state.selectedCategoryIds.includes(id)) state.selectedCategoryIds.push(id); }
                    else { state.selectedCategoryIds = state.selectedCategoryIds.filter(x => x !== id); }
        ...
        searchInput?.addEventListener('input', (e) => {
            ...
            const filtered = state.allCategories.filter(c => c.name.toLowerCase().includes(query));
            render(filtered);
        });
    ...
    function renderWordsDropdown(words) {
        ...
        const searchInput = elements.wordsDropdown.querySelector('.dropdown-search-input');

        const render = (wds) => {
            listContainer.innerHTML = '';
            wds.forEach(word => {
                const isSelected = state.selectedWordIds.includes(parseInt(word.id));
                const item = createDropdownItem(word.id, word.term.split('(')[0].trim(), isSelected, (id, selected) => {
                    if (selected) { if (!state.selectedWordIds.includes(id)) state.selectedWordIds.push(id); }
                    else { state.selectedWordIds = state.selectedWordIds.filter(x => x !== id); }
    ...
    function renderLevelsDropdown(levels) {
        ...
        levels.forEach(level => {
            ...
            const item = createDropdownItem(level, level, isSelected, (val, selected) => {
                if (selected) { if (!state.selectedLevels.includes(val)) state.selectedLevels.push(val); }
                else { state.selectedLevels = state.selectedLevels.filter(x => x !== val); }
    ...
    function updateToggleText(container, count, type) {
        ...
        display.textContent = count === 0 ? `Select ${type}...` : `${count} ${type} selected`;
            ...
            reader.onload = function(event) {
                ...
                img.onload = function() {
                    ...
                    if (elements.changeImageBtn) {
                        elements.changeImageBtn.style.display = 'block';
                    }
                    elements.hiddenImageInput.value = dataUrl;
                    
                    console.log("Image compressed and set. Length:", dataUrl.length);
                };
